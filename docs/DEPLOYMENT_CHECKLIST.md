...
Current deployment target: `1.1.0`
...
1. Confirm latest `main` checks are green in GitHub Actions:
   ...
   - `Coverage`
   - `Pkgdown`
2. Confirm release tag `1.1.0` exists and is published (not draft/prerelease).
3. Confirm README install snippet is pinned to `ref = "1.1.0"`.
...
2. Do not retag or rewrite `1.1.0`.
3. Create a hotfix commit on `main`.
4. Bump package version and notes for the next patch release (for example `1.1.1`).
