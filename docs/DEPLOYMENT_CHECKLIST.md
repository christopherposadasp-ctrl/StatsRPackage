...
Current deployment target: `0.1.2`
...
1. Confirm latest `main` checks are green in GitHub Actions:
   ...
   - `Coverage`
   - `Pkgdown`
2. Confirm release tag `0.1.2` exists and is published (not draft/prerelease).
3. Confirm README install snippet is pinned to `ref = "0.1.2"`.
...
2. Do not retag or rewrite `0.1.2`.
3. Create a hotfix commit on `main`.
4. Bump package version and notes for the next patch release (for example `0.1.3`).
